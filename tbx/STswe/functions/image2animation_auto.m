...
file_name = {D.name}.';
file_path2 = path_PL_archive_animate;
...
file_name2 = [region_str '_' yyyymm '_' type_str '.gif'];
...
for i=1:length(file_name)
    if strcmpi('gif',file_name{i}(end-2:end))
        ...
        a=imread([fullfile(file_path,file_name{i})]);
        [M  c_map]= rgb2ind(a,256);
    end
    if i==1
        imwrite(M,c_map,[fullfile(file_path2,file_name2)],'gif','LoopCount',loops,'DelayTime',delay1)
    elseif i==length(file_name)
        imwrite(M,c_map,[fullfile(file_path2,file_name2)],'gif','WriteMode','append','DelayTime',delay2)
    else
        imwrite(M,c_map,[fullfile(file_path2,file_name2)],'gif','WriteMode','append','DelayTime',delay)
    end
