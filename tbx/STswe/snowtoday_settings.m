...
%%% columns of the TIME Structure (should not have to change)
year_col = 1;
month_col = 2;
day_col = 3;
...
path_shp_huc06 = '';
path_shp_huc08 = '';
path_shp_ecoregions = fullfile(path_data, 'Ecoregions', 'westUSA_mtns3.shp');
path_shp_CA_prov = fullfile(path_data, 'Political', 'PROVINCE.SHP');
...
path_tab_political_CA = fullfile(path_data, 'political_ca.txt');
...
AOI_POLITICAL_COUNTRY = {'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'US', 'CA'};
AOI_POLITICAL = [0 2 4 6 8 16 30 31 32 35 41 46 49 53 56 02];  % FIPS codes for state/province or county. 0 = USWEST. For canada, 100 is added (E.g., 102 = BC, since FIPS=2 for BC)
AOI_HYDRO = [10 11 13 14 15 16 17 18 19];   % HUC2 codes
...
%%% cat into single AOI vector, where we cat the country name to political
%%% zones and cat "HUC" to hydro codes
% AOI.ID = [-1.*AOI_POLITICAL AOI_HYDRO];

AOI.ID = cell(1,numel(AOI_POLITICAL_COUNTRY)+numel(AOI_HYDRO));

% Loop through the arrays and combine the strings with numbers
for i = 1:numel(AOI.ID)
    if i<=numel(AOI_POLITICAL_COUNTRY)
        AOI.ID{i} = [AOI_POLITICAL_COUNTRY{i}, num2str(AOI_POLITICAL(i))];
    else
        AOI.ID{i} = ['HUC', num2str(AOI_HYDRO(i-numel(AOI_POLITICAL_COUNTRY)))];
    end
end
